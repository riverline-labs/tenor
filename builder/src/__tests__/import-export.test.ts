...
describe("importTenorFile", () => {
  it("returns an InterchangeBundle via WASM elaboration", async () => {
    const bundle = await importTenorFile("fact foo { type: Bool }");
    expect(bundle).toBeDefined();
    expect(bundle.kind).toBe("Bundle");
  });

  it("passes filename to the evaluator", async () => {
    const { evaluatorApi } = await import("@/wasm/evaluator");
    const elaborateMock = vi.mocked(evaluatorApi.elaborate);
    elaborateMock.mockClear();
    await importTenorFile("fact bar { type: Int }", "my-contract.tenor");
    expect(elaborateMock).toHaveBeenCalledWith("my-contract.tenor", "fact bar { type: Int }");
  });
});
