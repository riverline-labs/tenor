 ...
 *
 * Initializes the WASM elaborator, runs the 6-pass pipeline on the source,
 * and returns the resulting interchange bundle.
 */
export async function importTenorFile(
  dslContent: string,
  fileName: string = "contract.tenor"
): Promise<InterchangeBundle> {
  await initEvaluator();
  const interchangeJson = evaluatorApi.elaborate(fileName, dslContent);
  return importInterchangeJson(interchangeJson);
}
