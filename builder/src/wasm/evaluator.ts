...
export interface EvaluatorApi {
  ...
  elaborate(filename: string, source: string): string;
}
...
export const evaluatorApi: EvaluatorApi = {
  ...
  elaborate(filename: string, source: string): string {
    const wasm = getWasm();
    const result = wasm.elaborate(filename, source);
    const parsed = parseResult<{ bundle: unknown }>(result);
    return JSON.stringify(parsed.bundle);
  },
};
