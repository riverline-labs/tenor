...
import { useFacts, useEntities } from "@/store/contract";
...
function TypeInput({ type, value, onChange, compact }: TypeInputProps) {
  const inputCls = `w-full rounded-md border border-border px-2 py-1 text-sm focus:border-ring focus:outline-none${compact ? " py-0.5" : ""}`;
      ...
      return (
        <label className="flex cursor-pointer items-center gap-2">
          <input
            ...
            className="h-4 w-4 rounded-md border-border text-primary"
          />
      ...
      return (
        <div className="space-y-1">
          {arr.map((item, i) => (
            <div key={i} className="flex items-start gap-1">
              ...
              <button
                ...
                className="mt-0.5 rounded-md px-1 py-0.5 text-xs text-destructive hover:bg-destructive/10"
                title="Remove item"
              >
                ×
              </button>
          ...
          <button
            ...
            className="rounded-md border border-dashed border-border px-2 py-0.5 text-xs text-muted-foreground hover:border-ring hover:text-primary disabled:opacity-40"
          >
            + Add item
          </button>
          {listType.max && (
...
function FactRow({ fact, value, onChange }: FactRowProps) {
  return (
    <div className="group rounded-md border border-border bg-card p-3 hover:border-primary/50">
      <div className="mb-1.5 flex items-center justify-between">
        <span className="font-mono text-sm font-medium text-foreground">
          {fact.id}
        </span>
        <span className="text-xs text-muted-foreground">{fact.type.base}</span>
      </div>
...
export function FactInputPanel({ compact, onEvaluate }: FactInputPanelProps) {
  ...
  if (facts.length === 0 && entities.length === 0) {
    return (
      <div className="flex h-40 items-center justify-center rounded-md border-2 border-dashed border-border text-sm text-muted-foreground">
  ...
  return (
    <div className="flex h-full flex-col">
          ...
          <section className="mt-4">
            ...
            <div className="space-y-2">
              {entities.map((entity: EntityConstruct) => (
                ...
                >
                  <span className="w-32 shrink-0 font-mono text-sm text-foreground">
                    {entity.id}
                  </span>
                  <select
      ...
      {/* Action buttons */}
      <div className={`flex gap-2 border-t border-border bg-muted ${compact ? "p-2" : "p-3"}`}>
        <Button
