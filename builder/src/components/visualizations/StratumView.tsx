...
function RuleCard({ rule, selected, issues, compact, onClick }: RuleCardProps) {
  ...
  return (
    <button
      ...
      title={ruleIssues.map((i) => i.message).join("\n") || undefined}
    >
      <div className="mb-0.5 font-mono font-semibold text-foreground truncate">
        {rule.id}
      </div>
      <div className="text-muted-foreground truncate">
        {summarizeExpr(rule.body.when)}
      </div>
      <div className="mt-0.5 truncate font-mono text-primary">
        ...
        return (
          <div key={stratum} className={`rounded border ${color}`}>
            ...
            >
              ...
              <span className="rounded-full bg-card/60 px-1.5 py-0.5 text-xs font-medium">
