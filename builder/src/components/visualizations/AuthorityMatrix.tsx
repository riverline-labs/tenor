...
export function AuthorityMatrix({ personas, operations }: AuthorityMatrixProps) {
  ...
  if (personas.length === 0 || operations.length === 0) {
    return (
      <div className="rounded border-2 border-dashed border-border py-6 text-center text-xs text-muted-foreground">
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        <thead>
          <tr>
            {/* Corner cell */}
            <th className="w-32 border-b border-r border-border bg-muted px-2 py-1.5 text-left text-xs font-semibold text-muted-foreground">
              Persona \ Op
            </th>
            {operations.map((op) => {
              ...
              return (
                <th
                  key={op.id}
                  className={`border-b border-l border-border px-2 py-1.5 text-center font-mono font-medium ${
                    isEmpty
            ...
            <th className="border-b border-l border-border bg-muted px-2 py-1.5 text-center font-semibold text-muted-foreground">
              Total ops
            </th>
          </tr>
        </thead>
        ...
        <tbody>
          {personas.map((persona) => {
            ...
            return (
              <tr
                key={persona.id}
                className={isEmpty ? "bg-warning/10" : "hover:bg-muted"}
              >
                {/* Persona label */}
                <td
                  className={`border-b border-r border-border px-2 py-1.5 font-mono font-medium ${
                    isEmpty ? "text-warning" : "text-foreground"
                  }`}
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    {isEmpty && (
                      <span className="rounded bg-warning/20 px-1 py-0.5 text-xs text-warning">
                        unused
                      </span>
                    )}
                  </div>
                ...
                {operations.map((op) => {
                  ...
                  return (
                    <td
                      key={op.id}
                      className={`border-b border-l border-border px-2 py-1.5 text-center ${
                        authorized
                          ? "bg-success/10"
                    ...
                    >
                      <button
                        ...
                        className={`flex h-6 w-6 items-center justify-center rounded transition-colors mx-auto ${
                          authorized
                            ? "bg-success text-card hover:bg-success/80"
                            : "border border-border text-border hover:border-success hover:text-success"
                        }`}
                        title={
                          authorized
                ...
                <td
                  className={`border-b border-l border-border px-2 py-1.5 text-center font-semibold ${
                    isEmpty
                      ? "bg-warning/10 text-warning"
                      : "bg-muted text-foreground"
                  }`}
          ...
          <tr className="bg-muted">
            <td className="border-t border-r border-border px-2 py-1.5 text-xs font-semibold text-muted-foreground">
              Total personas
            </td>
            {operations.map((op) => {
              const count = personas.filter((p) =>
              ...
              return (
                <td
                  key={op.id}
                  className={`border-t border-l border-border px-2 py-1.5 text-center font-semibold ${
                    count === 0 ? "text-warning" : "text-foreground"
                  }`}
                >
                  {count}
                </td>
            ...
            {/* Grand total cell */}
            <td className="border-t border-l border-border px-2 py-1.5 text-center font-semibold text-muted-foreground">
              —
            </td>
          </tr>
        </tbody>
