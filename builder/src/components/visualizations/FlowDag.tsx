  ...
  return (
    <defs>
      <marker
        ...
        orient="auto"
      >
        <polygon points="0 0, 8 3, 0 6" fill={THEME_COLORS.primary} />
      </marker>
      <marker
        id="arrow-gray"
        markerWidth="8"
        ...
        orient="auto"
      >
        <polygon points="0 0, 8 3, 0 6" fill={THEME_COLORS.mutedForeground} />
      </marker>
      <marker
        id="arrow-red"
        markerWidth="8"
        ...
        orient="auto"
      >
        <polygon points="0 0, 8 3, 0 6" fill={THEME_COLORS.destructive} />
      </marker>
    </defs>
  );
}
...
function EdgePath({ edge, fromNode, toNode, onClick }: EdgeProps) {
  ...
  const color = edge.isFailure ? THEME_COLORS.destructive : THEME_COLORS.mutedForeground;
...
function OperationStepNode({ node, step, isSelected, isHighlighted, isEntry, onClick }: StepNodeProps) {
  ...
  const borderColor = isSelected ? THEME_COLORS.primary : isHighlighted ? THEME_COLORS.accent : THEME_COLORS.primary;
  const bgColor = isHighlighted ? THEME_COLORS.muted : isSelected ? THEME_COLORS.muted : THEME_COLORS.muted;
  const glow = isHighlighted ? `drop-shadow(0 0 6px ${THEME_COLORS.accent})` : undefined;

  return (
    <g transform={`translate(${node.x}, ${node.y})`} onClick={onClick} className="cursor-pointer" style={{ filter: glow }}>
      {isEntry && (
        <>
          <circle cx={-16} cy={STEP_H / 2} r={6} fill={THEME_COLORS.primary} />
          <path d={`M -10 ${STEP_H / 2} L -2 ${STEP_H / 2}`} stroke={THEME_COLORS.primary} strokeWidth={1.5} markerEnd="url(#arrow-blue)" />
        </>
      )}
      <rect
        width={STEP_W}
      ...
      />
      <text x={8} y={18} fontSize={10} fill={THEME_COLORS.foreground} fontWeight={600} className="select-none">
        {s.op}
      </text>
      <text x={8} y={32} fontSize={9} fill={THEME_COLORS.mutedForeground} className="select-none">
        persona: {s.persona || "—"}
      </text>
      <text x={8} y={46} fontSize={9} fill={THEME_COLORS.mutedForeground} className="select-none">
...
function BranchStepNode({ node, step, isSelected, isHighlighted, isEntry, onClick }: StepNodeProps) {
  ...
  const borderColor = isSelected ? THEME_COLORS.warning : THEME_COLORS.warning;
  const bgColor = isSelected ? THEME_COLORS.muted : THEME_COLORS.muted;
  const hw = STEP_W / 2;
  const hh = STEP_H / 2;
  const glow = isHighlighted ? `drop-shadow(0 0 6px ${THEME_COLORS.accent})` : undefined;
  ...
  return (
    <g transform={`translate(${node.x}, ${node.y})`} onClick={onClick} className="cursor-pointer" style={{ filter: glow }}>
      {isEntry && (
        <>
          <circle cx={-16} cy={hh} r={6} fill={THEME_COLORS.primary} />
          <path d={`M -10 ${hh} L -2 ${hh}`} stroke={THEME_COLORS.primary} strokeWidth={1.5} markerEnd="url(#arrow-blue)" />
        </>
      )}
      <polygon
        points={points}
        ...
        strokeWidth={isSelected ? 2 : 1.5}
      />
      <text x={hw} y={hh - 6} fontSize={9} fill={THEME_COLORS.foreground} textAnchor="middle" className="select-none">
        {condText.length > 18 ? condText.slice(0, 16) + "..." : condText}
      </text>
      <text x={hw} y={hh + 8} fontSize={8} fill={THEME_COLORS.mutedForeground} textAnchor="middle" className="select-none">
        T / F
      </text>
    </g>
  );
...
function HandoffStepNode({ node, step, isSelected, isHighlighted, isEntry, onClick }: StepNodeProps) {
  ...
  const borderColor = isSelected ? THEME_COLORS.success : THEME_COLORS.success;
  const bgColor = isSelected ? THEME_COLORS.muted : THEME_COLORS.muted;
  const glow = isHighlighted ? `drop-shadow(0 0 6px ${THEME_COLORS.accent})` : undefined;
  ...
  return (
    <g transform={`translate(${node.x}, ${node.y})`} onClick={onClick} className="cursor-pointer" style={{ filter: glow }}>
      ...
      <rect
        ...
        strokeWidth={isSelected ? 2 : 1.5}
      />
      <text x={STEP_W / 2} y={STEP_H / 2 - 4} fontSize={10} fill={THEME_COLORS.foreground} textAnchor="middle" fontWeight={600} className="select-none">
        handoff
      </text>
      <text x={STEP_W / 2} y={STEP_H / 2 + 10} fontSize={9} fill={THEME_COLORS.mutedForeground} textAnchor="middle" className="select-none">
...
function SubFlowStepNode({ node, step, isSelected, isHighlighted, isEntry, onClick }: StepNodeProps) {
  ...
  const borderColor = isSelected ? THEME_COLORS.accent : THEME_COLORS.accent;
  const bgColor = isSelected ? THEME_COLORS.muted : THEME_COLORS.muted;
  const glow = isHighlighted ? `drop-shadow(0 0 6px ${THEME_COLORS.accent})` : undefined;
...
function ParallelStepNode({ node, step, isSelected, isHighlighted, isEntry, onClick }: StepNodeProps) {
  ...
  const borderColor = isSelected ? THEME_COLORS.primary : THEME_COLORS.primary;
  const bgColor = isSelected ? THEME_COLORS.muted : THEME_COLORS.muted;
  const glow = isHighlighted ? `drop-shadow(0 0 6px ${THEME_COLORS.accent})` : undefined;
  ...
  return (
    <g transform={`translate(${node.x}, ${node.y - (laneH - STEP_H) / 2})`} onClick={onClick} className="cursor-pointer" style={{ filter: glow }}>
      {isEntry && (
        <>
          <circle cx={-16} cy={laneH / 2} r={6} fill={THEME_COLORS.primary} />
          <path d={`M -10 ${laneH / 2} L -2 ${laneH / 2}`} stroke={THEME_COLORS.primary} strokeWidth={1.5} markerEnd="url(#arrow-blue)" />
        </>
      )}
      ...
      {s.branches.map((branch, i) => (
        <g key={branch.id}>
          ...
          <text x={6} y={(i + 0.5) * (laneH / branchCount) + 4} fontSize={9} fill={THEME_COLORS.foreground} className="select-none">
            {branch.id}
          </text>
        </g>
      ))}
      <text x={laneW / 2} y={-8} fontSize={9} fill={THEME_COLORS.mutedForeground} textAnchor="middle" fontWeight={600} className="select-none">
...
function TerminalStepNode({
  ...
  const borderColor = isSuccess ? THEME_COLORS.success : THEME_COLORS.destructive;
  const bgColor = isSuccess ? THEME_COLORS.muted : THEME_COLORS.muted;
  const textColor = isSuccess ? THEME_COLORS.success : THEME_COLORS.destructive;
...
export function FlowDag({
  ...
  if (steps.length === 0) {
    return (
      <div className="flex h-40 items-center justify-center rounded border-2 border-dashed border-border text-sm text-muted-foreground">
  ...
  return (
    <div className="overflow-hidden rounded border border-border bg-card">
          ...
          return (
            ...
            >
              ...
              <text
                ...
                textAnchor="middle"
                fontSize={9}
                fill={THEME_COLORS.mutedForeground}
                className="select-none pointer-events-none"
              >
                {step.id}
              </text>
      ...
      {/* Zoom controls */}
      {editable && (
        <div className="flex items-center gap-1 border-t border-border bg-muted px-3 py-1">
          <button
