...
export function StateMachine({
  ...
  if (states.length === 0) {
    return (
      <div className="flex h-48 items-center justify-center rounded border-2 border-dashed border-border text-sm text-muted-foreground">
  ...
  return (
    <div className="relative rounded border border-border bg-card">
      ...
      {addMode && (
        <div className="absolute left-0 right-0 top-0 z-10 rounded-t bg-primary px-3 py-1 text-center text-xs text-card">
          {addSource === null
            ? "Click source state..."
      ...
      >
        <defs>
          <marker
            ...
            orient="auto"
          >
            <polygon points="0 0, 10 3.5, 0 7" fill={THEME_COLORS.mutedForeground} />
          </marker>
          <marker
            ...
            orient="auto"
          >
            <polygon points="0 0, 10 3.5, 0 7" fill={THEME_COLORS.destructive} />
          </marker>
          <marker
            id="arrowhead-blue"
            markerWidth="10"
            ...
            orient="auto"
          >
            <polygon points="0 0, 10 3.5, 0 7" fill={THEME_COLORS.primary} />
          </marker>
        </defs>
        ...
        >
          ...
          {effectiveEdges.map((edge, idx) => {
            ...
            const color = isSelected ? THEME_COLORS.destructive : THEME_COLORS.mutedForeground;
          ...
          {effectiveNodes.map((node) => {
            ...
            let strokeColor: string = THEME_COLORS.border;
            let fillColor: string = THEME_COLORS.card;
            let strokeWidth = 1.5;

            if (isInitial) {
              strokeColor = THEME_COLORS.primary;
              fillColor = THEME_COLORS.muted;
            }
            if (isSelected) {
              strokeColor = THEME_COLORS.destructive;
              strokeWidth = 2.5;
            }
            if (isAddSource) {
            ...
            return (
              ...
              >
                ...
                <text
                  ...
                  fill={isInitial ? THEME_COLORS.primary : THEME_COLORS.foreground}
