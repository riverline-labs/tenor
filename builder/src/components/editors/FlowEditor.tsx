    ...
    <div className="flex items-center gap-1">
      ...
      {mode === "step" && (
        <select
          ...
          className="flex-1 rounded-md border border-border px-1 py-0.5 text-xs"
        >
...
function FailureHandlerEditor({ value, onChange, stepIds, selfId, personas }: FailureHandlerEditorProps) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-1">
        <span className="text-xs text-muted-foreground">on_failure:</span>
        <select
          value={kind}
          onChange={(e) => {
      ...
      {kind === "Escalate" && (
        <div className="space-y-0.5 pl-2">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs text-muted-foreground">to_persona:</span>
            <select
  ...
  return (
    <div className="space-y-1 pl-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-muted-foreground">Compensation steps</span>
        <button
          onClick={addCompStep}
          className="rounded-md border border-dashed border-border px-1.5 py-0.5 text-xs text-muted-foreground hover:bg-muted"
        >
          + Add
        </button>
      </div>
      {value.steps.map((cs, idx) => (
        <div key={idx} className="flex items-center gap-1 rounded-md border border-border bg-muted p-1">
          <input
            type="text"
            value={cs.op}
            onChange={(e) => updateCompStep(idx, { ...cs, op: e.target.value })}
            className="w-24 rounded-md border border-border px-1 py-0.5 text-xs font-mono"
            placeholder="op_id"
          />
          <input
            type="text"
...
function OperationStepDetail({ step, allStepIds, operations, personas, onChange, onDelete }: OperationStepDetailProps) {
  ...
  return (
    <div className="space-y-3 p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-primary uppercase tracking-wide">OperationStep</span>
        <button onClick={onDelete} className="rounded-md px-2 py-0.5 text-xs text-destructive hover:bg-destructive/10">Delete</button>
      </div>

      <div className="flex items-center gap-2">
        <label className="w-16 text-xs text-muted-foreground">Step ID</label>
        <input
          type="text"
          value={step.id}
      ...
      <div className="flex items-center gap-2">
        <label className="w-16 text-xs text-muted-foreground">Operation</label>
        <select
          value={step.op}
          onChange={(e) => {
      ...
      <div className="flex items-center gap-2">
        <label className="w-16 text-xs text-muted-foreground">Persona</label>
        <select
          value={step.persona}
      ...
      <div>
        <div className="mb-1 flex items-center justify-between">
          <span className="text-xs font-medium text-muted-foreground">Outcome routing</span>
          <button onClick={addOutcomeRoute} className="rounded-md border border-dashed border-border px-1.5 py-0.5 text-xs text-muted-foreground hover:bg-muted">
            + Add
          </button>
        </div>
        {outcomeKeys.map((label) => (
          <div key={label} className="mb-1 flex items-center gap-1">
            <input
              ...
              className="w-24 rounded-md border border-border px-1 py-0.5 font-mono text-xs"
            />
            ...
            <StepTargetEditor
              ...
              label=""
            />
            <button onClick={() => removeOutcomeRoute(label)} className="rounded-md px-1 py-0.5 text-xs text-destructive hover:bg-destructive/10">×</button>
          </div>
        ))}
      </div>
...
function BranchStepDetail({ step, allStepIds, personas, facts, verdicts, onChange, onDelete }: BranchStepDetailProps) {
  return (
    <div className="space-y-3 p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-warning uppercase tracking-wide">BranchStep</span>
        <button onClick={onDelete} className="rounded-md px-2 py-0.5 text-xs text-destructive hover:bg-destructive/10">Delete</button>
      </div>
...
function HandoffStepDetail({ step, allStepIds, personas, onChange, onDelete }: HandoffStepDetailProps) {
  return (
    <div className="space-y-3 p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-primary uppercase tracking-wide">HandoffStep</span>
        <button onClick={onDelete} className="rounded-md px-2 py-0.5 text-xs text-destructive hover:bg-destructive/10">Delete</button>
      </div>

      <div className="flex items-center gap-2">
        <label className="w-20 text-xs text-muted-foreground">Step ID</label>
        <input
          type="text"
          value={step.id}
          onChange={(e) => onChange({ ...step, id: e.target.value })}
      ...
      <div className="flex items-center gap-2">
        <label className="w-20 text-xs text-muted-foreground">from_persona</label>
        <select
      ...
      <div className="flex items-center gap-2">
        <label className="w-20 text-xs text-muted-foreground">to_persona</label>
        <select
      ...
      <div className="flex items-center gap-2">
        <label className="w-20 text-xs text-muted-foreground">next</label>
        <select
          value={step.next}
...
function SubFlowStepDetail({ step, allStepIds, personas, flows, onChange, onDelete }: SubFlowStepDetailProps) {
  return (
    <div className="space-y-3 p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-chart-2 uppercase tracking-wide">SubFlowStep</span>
        <button onClick={onDelete} className="rounded-md px-2 py-0.5 text-xs text-destructive hover:bg-destructive/10">Delete</button>
      </div>
      ...
      <div className="flex items-center gap-2">
        <label className="w-16 text-xs text-muted-foreground">Flow</label>
        <select
          value={step.flow}
...
function ParallelStepDetail({ step, allStepIds, onChange, onDelete }: ParallelStepDetailProps) {
  ...
  return (
    <div className="space-y-3 p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-primary uppercase tracking-wide">ParallelStep</span>
        <button onClick={onDelete} className="rounded-md px-2 py-0.5 text-xs text-destructive hover:bg-destructive/10">Delete</button>
      </div>
      ...
      <div>
        <div className="mb-1 flex items-center justify-between">
          <span className="text-xs font-medium text-muted-foreground">Branches ({step.branches.length})</span>
          <button onClick={addBranch} className="rounded-md border border-dashed border-border px-1.5 py-0.5 text-xs text-muted-foreground hover:bg-muted">
            + Branch
          </button>
        </div>
        {step.branches.map((branch, idx) => (
          <div key={branch.id} className="mb-1 flex items-center gap-1 rounded-md border border-border bg-muted p-1">
            <input
              type="text"
              value={branch.id}
            ...
            />
            <button onClick={() => removeBranch(idx)} className="rounded-md px-1 py-0.5 text-xs text-destructive hover:bg-destructive/10">×</button>
          </div>
        ))}
      </div>
...
export function FlowEditor() {
  ...
  const STEP_KINDS: { kind: StepKind; label: string; color: string }[] = [
    { kind: "OperationStep", label: "Operation Step", color: "text-primary" },
    { kind: "BranchStep", label: "Branch Step", color: "text-warning" },
    { kind: "HandoffStep", label: "Handoff Step", color: "text-primary" },
    { kind: "SubFlowStep", label: "Sub-Flow Step", color: "text-chart-2" },
    { kind: "ParallelStep", label: "Parallel Step", color: "text-primary" },
  ];

  return (
    <div className="flex h-full overflow-hidden">
      {/* Flow list sidebar */}
      <aside className="flex w-48 shrink-0 flex-col border-r border-border bg-muted">
        <div className="flex items-center justify-between border-b border-border px-3 py-2">
          <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Flows</span>
          <Button
        ...
        <div className="flex-1 overflow-y-auto">
          {flows.length === 0 ? (
            <div className="p-3 text-center text-xs text-muted-foreground">No flows yet</div>
          ) : (
            flows.map((flow) => (
              <button
                key={flow.id}
              ...
              >
                ...
                <div className="text-muted-foreground">{flow.steps.length} step{flow.steps.length !== 1 ? "s" : ""}</div>
              </button>
            ))
          )}
        </div>
      ...
      {!selectedFlow ? (
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex flex-1 flex-col overflow-y-auto">
            ...
            {/* Step toolbar */}
            <div className="shrink-0 flex items-center gap-2 border-b border-border bg-muted px-3 py-2">
              {/* Add step dropdown */}
              <div className="relative">
                ...
                {showAddStepDropdown && (
                  <div className="absolute left-0 top-full z-10 mt-1 w-44 rounded-md border border-border bg-card shadow-lg">
                    {STEP_KINDS.map(({ kind, label, color }) => (
                      <button
                        key={kind}
                        onClick={() => addStep(kind)}
                        className={`block w-full px-3 py-1.5 text-left text-xs hover:bg-muted ${color}`}
                      >
                        {label}
                      </button>
                    ))}
          ...
          {selectedStep && (
            <aside className="flex w-72 shrink-0 flex-col overflow-y-auto border-l border-border bg-card">
              <div className="border-b border-border px-3 py-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-foreground">Step Editor</span>
                  <button
