...
const selectClass = "flex-1 rounded-md border border-input bg-background px-2 py-1 text-sm";
const inputClass = "flex-1 rounded-md border border-input bg-background px-2 py-1 text-sm";
    ...
    <div className="flex items-center gap-1">
      ...
      {kind === "fact_ref" && (
        <select
          ...
          className={selectClass}
        >
          {facts.length === 0 && <option value="">&mdash; no facts &mdash;</option>}
          {facts.map((f) => (
    ...
    <div className="flex items-center gap-1">
      ...
      {selected !== current && (
        <button
          ...
          className="rounded-md bg-primary/20 px-1.5 py-0.5 text-xs text-primary hover:bg-primary/30"
        >
          Replace
        </button>
      )}
...
function depthColor(depth: number): string {
  return DEPTH_OPACITIES[depth % DEPTH_OPACITIES.length];
}
...
function ExpressionNode({
    ...
    return (
      <div className={`rounded-md border p-2 ${colorClass}`}>
        ...
        <div className="space-y-1">
          ...
          />
          <div className="flex items-center gap-1">
            <span className="w-8 text-xs text-muted-foreground">op</span>
            <select
              value={cmp.op}
              onChange={(e) =>
    ...
    return (
      <div className={`rounded-md border p-2 ${colorClass}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {verdicts.length === 0 && (
              <option value="">&mdash; no verdicts available &mdash;</option>
            )}
            {verdicts.map((v) => (
              <option key={v} value={v}>
                {v}
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-20 rounded-md border border-input bg-background px-2 py-1 font-mono text-sm"
              placeholder="x"
            />
            ...
            >
              {listFacts.length === 0 && (
                <option value="">&mdash; no List facts &mdash;</option>
              )}
  ...
  return (
    <div className={`rounded-md border p-2 ${colorClass}`}>
      {header}
      <p className="text-xs text-muted-foreground">Unknown expression type</p>
    </div>
  );
}
  ...
  if (value === null) {
    return (
      <div className="rounded-md border-2 border-dashed border-border p-4 text-center">
        <p className="mb-2 text-xs text-muted-foreground">No condition &mdash; always true</p>
        <ExprTypeDropdown
...
export function ExprTypeDropdown({
  ...
  return (
    <div className="relative inline-block">
      <button
        onClick={() => setOpen((o) => !o)}
        className="rounded-md border border-dashed border-border px-3 py-1 text-xs text-muted-foreground hover:bg-muted"
      >
        + Add expression
      </button>
      {open && (
        <div className="absolute left-0 top-full z-10 mt-1 w-56 rounded-md border border-border bg-popover shadow-lg">
          {options.map(({ type, label }) => (
            <button
              ...
              className="block w-full px-3 py-1.5 text-left text-xs text-popover-foreground hover:bg-muted"
            >
              {label}
            </button>
          ))}
