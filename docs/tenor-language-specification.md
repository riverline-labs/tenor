...
In the multi-outcome form, each effect tuple is extended with a fourth element (the OutcomeLabel) to declare which outcome it belongs to. All effects for a given outcome are applied atomically when that outcome is produced.
...
operation reject_requisition {
  ...
  precondition:     verdict_present(rejection_authorized)
...
operation submit_requisition {
  allowed_personas: [requestor]
  precondition:     requisition_total present and
