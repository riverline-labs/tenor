...
#[wasm_bindgen]
pub fn elaborate(filename: &str, source: &str) -> String {
    let mut files = HashMap::new();
    files.insert(PathBuf::from(filename), source.to_string());
    let provider = tenor_core::InMemoryProvider::new(files);
    match tenor_core::elaborate_with_provider(&PathBuf::from(filename), &provider) {
        Ok(bundle) => serde_json::json!({ "bundle": bundle }).to_string(),
        Err(e) => error_json(&e.message),
    }
}
