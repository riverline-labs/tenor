...
fn explain_tenor_file_exits_0() {
    tenor()
        .args([
            "explain",
            "conformance/fixtures/saas/saas_subscription.tenor",
        ])
        .assert()
        .success()
        .stdout(
...
fn e12_etag_determinism() {
    let contract = "conformance/fixtures/saas/saas_subscription.tenor";
...
fn e12_etag_change_detection() {
    ...
    let out2 = tenor()
        .args([
            "elaborate",
            "--manifest",
            "conformance/fixtures/healthcare/prior_auth.tenor",
        ])
        .output()
